// MLB Predictor Web App - React Frontend
// Phase 3: Added simulate all games feature
...
export default function App() {
  ...
  const exportCSV = () => {
    if (!results) return;
    const csv = `Home Team,Home Score,Away Team,Away Score\n${homeTeam},${results.homeScore},${awayTeam},${results.awayScore}`;
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    saveAs(blob, `mlb_prediction_${homeTeam}_vs_${awayTeam}.csv`);
  };
  ...
  const renderOddsMovement = (team) => {
    ...
    const color = movement.direction === "up" ? "text-red-600" : movement.direction === "down" ? "text-green-600" : "text-gray-500";
    return (
      <p className={`text-sm ${color}`}>{team} Odds: {odds.open} {arrow} {odds.current}</p>
    );
  };

  return (
    <main className="p-6 max-w-xl mx-auto space-y-6">
      <h1 className="text-3xl font-bold">MLB Game Simulator</h1>
      ...
      <Card>
        <CardContent className="space-y-4 pt-4">
          ...
          <Button onClick={simulateGame} className="w-full">
            <Play className="mr-2 h-4 w-4" /> Simulate Game
          </Button>

          <Button onClick={simulateAllGames} className="w-full mt-2">
            <Play className="mr-2 h-4 w-4" /> Simulate All Games
          </Button>
